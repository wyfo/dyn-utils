...
use syn::{
    parse_macro_input, parse_quote as p, parse_quote, visit_mut::VisitMut, CapturedParam, GenericParam, Generics, ItemImpl,
    ItemTrait, Lifetime, LifetimeParam, Receiver, TraitItem, Type,
...
fn with_storage_impl(r#trait: &ItemTrait) -> syn::Result<TokenStream> {
    ...
    impl_generics
        .params
        .push(p!(__Dyn: #trait_name #ty_generics));
    let impl_with_storage = ItemImpl {
        ...
        trait_: Some((None, with_storage_trait.ident.clone().into(), p!(for))),
